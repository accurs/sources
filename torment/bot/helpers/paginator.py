...
from bot.helpers.context import TormentContext, _emoji
...
class Paginator(View):
    ...
    def buttons(self):
        prev_btn = Button(
            ...
            emoji=_emoji("EMOJI_LEFT"),
        )
        prev_btn.callback = self.callback
        
        cancel_btn = Button(
            custom_id="paginator:cancel",
            style=ButtonStyle.secondary,
            emoji=_emoji("EMOJI_CANCEL"),
        )
        cancel_btn.callback = self.callback
        
        next_btn = Button(
            custom_id="paginator:next",
            style=ButtonStyle.secondary,
            emoji=_emoji("EMOJI_RIGHT"),
        )
        next_btn.callback = self.callback
        
        nav_btn = Button(
            custom_id="paginator:navigate",
            style=ButtonStyle.secondary,
            emoji=_emoji("EMOJI_SKIP"),
        )
        nav_btn.callback = self.callback
        
        return [prev_btn, cancel_btn, next_btn, nav_btn]
            ...
            for chunk in as_chunks(pages, per_page):
                ...
                for page in chunk:
                    if counter:
                        ...
                        prepared.description = (prepared.description or "") + f"{page}\n"
    ...
    async def start(self, **kwargs: Any) -> Message:
        ...
        if len(self.pages) == 1:
            self.message = (
                await self.ctx.reply(content=page, **kwargs)
                if isinstance(page, str)
                else await self.ctx.reply(embed=page, **kwargs)
            )
        else:
            self.message = (
                await self.ctx.reply(content=page, view=self, **kwargs)
                if isinstance(page, str)
                else await self.ctx.reply(embed=page, view=self, **kwargs)
            )
