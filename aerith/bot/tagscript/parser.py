...
from .classes import ENV, Converter, Node, ParsedTag, Tag
from ..utilities import get_color, ordinal


class Parser:
    def __init__(self, env: Optional[dict] = None, *, limit: Optional[int] = None):
        self.limit = limit
        self.env = env or {}
        self.tags = []
        self._delimiter = r"\:"
    ...
    def _base_argument_conversion(self, argument, converter):
        ...
        if converter is bool:
            if argument.lower() in ("enable", "true", "yes", "on"):
                return True
            return False
    ...
    def do_argument_conversion(self, argument, converter) -> Any:
        ...
        if origin is Union:
            for conv in get_args(converter):
    ...
    def parse_single_tag(self, tag) -> Optional[ParsedTag]:
        escape_re = f"(?<!{re.escape(self._escape_character)})"
        splitted = re.split(escape_re + self._delimiter, tag, 1)
        if len(splitted) < 2:
            tag_, args = splitted[0], ""
        else:
            tag_, args = splitted[:2]
        tag_body = re.split(escape_re + self._attribute_delimiter, tag_)

        buffer: Union[Parser, Tag] = self
        for iteration in tag_body:
            if got_tag := self.get_tag(iteration, parent=buffer):
                buffer = got_tag
                continue
            return None

        if not isinstance(buffer, Tag):
            return None
        ...
        arguments = re.split(escape_re + self._argument_delimiter, args)
    ...
    def parse(self, string, env=None, *, limit=None):
        ...
        return self.parse_nodes(nodes, string, ENV(env), limit)  # pyright: ignore[reportArgumentType, reportCallIssue]
...
@parser.tag("author")
def author(
    self, name: str, icon_url: Optional[str] = None, external_url: Optional[str] = None
) -> None:
...
def image(self, image_url: str) -> None:
    self.model.embed.set_image(url=image_url)

@parser.tag("thumbnail")
def thumbnail(self, image_url: str) -> None:
    self.model.embed.set_thumbnail(url=image_url)

@parser.tag("field")
def field(self, name: str, value: str, inline: bool = False) -> None:
...
@parser.tag("footer")
def footer(self, text: str, icon_url: Optional[str] = None) -> None:
...
def timestamp(self, value: Optional[str] = None) -> None:
    self.model.embed.timestamp = utcnow() if not value else dateparser.parse(value)

@parser.tag("button")
def button(
    self, option: str, label: str, emoji: Optional[str] = None, disabled: bool = False
) -> None:
    if emoji == "disabled":
        emoji = None
        disabled = True

    if match := regex.URL.match(option):
        ...
        style = getattr(ButtonStyle, option, ButtonStyle.secondary)
        self.model.view.add_item(
            Button(
                style=style,
                label=label,
...
def compile(
    ...
    **vars: Any,
    ...
    user: Optional[Member] = vars.get("user")
    guild: Optional[Guild] = vars.get("guild")
    channel: Optional[Union[TextChannel, VoiceChannel]] = vars.get("channel")
    ...
    if user is not None:
        ...
        joined_at_str = (
            user.joined_at.strftime("%m/%d/%Y, %I:%M %p") if user.joined_at else "N/A"
        )
        joined_at_ts = str(int(user.joined_at.timestamp())) if user.joined_at else "N/A"
        ...
        script = (
            ...
            .replace("{user.joined_at}", joined_at_str)
            .replace("{user.joined_at_timestamp}", joined_at_ts)
            .replace(
            ...
            .replace(
                "{user.top_role}",
                user.top_role.name
                if guild and user.top_role != guild.default_role
                else "N/A",
            )
            .replace(
                "{user.role_list}",
        ...
        join_pos = getattr(user, "join_position", None)
        if join_pos is not None:
            script = script.replace("{user.join_position}", str(join_pos)).replace(
                "{user.join_position_suffix}", ordinal(join_pos)
            )
    ...
    if guild is not None:
        script = (
            ...
            .replace(
                "{guild.key_features}",
                ", ".join(f.replace("_", " ").title() for f in guild.features) or "N/A",
            )
