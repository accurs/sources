...
async def is_donor(interaction):
    ...
    if cached_result is not None:
        ...
        if not is_donor:
            await interaction.response.send_message("<:premium:1311062205650833509> This is a premium-only command. Run </premium perks:1368369692473036879> to learn more.", ephemeral=True)
        return is_donor
...
async def is_creator(interaction):
    creator_ids = ["1326213864391442504", "935448935777050645", "1363295564133040272", "1173788423308451841"]
    if str(interaction.user.id) not in creator_ids:
