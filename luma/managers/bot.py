...
class Luma(commands.Bot):
  def __init__(self: "Luma"):
    ...
    self.cache = Cache()
    self.embed = Embed()
    self.login_data = {m: os.environ[m] for m in ['host', 'port', 'user', 'database', 'password']}
  ...
  async def load_modules(self: "Luma"):
    files = [
      f"{x}.{p[:-3]}"
      for x in ["cogs", "events"] for p in os.listdir(f"./{x}")
      if p.endswith(".py")
    ]
    for file in files:
      try:
  ...
  async def on_ready(self: "Luma"):
    ...
    discord.User.activity = property(
      fget=lambda a: a.mutual_guilds[0].get_member(a.id).activity if a.mutual_guilds else None
    )
  ...
  def files(self) -> List[str]:
    return [
      f"{root}/{f}" for root, _, file in os.walk('./')
      for f in file
      if f.endswith('.py')
    ]
  ...
  async def on_command(self: "Luma", ctx: Context):
    logging.info(f"{ctx.author} ({ctx.author.id}) executed {ctx.command} in {ctx.guild} ({ctx.guild.id}). -> {ctx.message.content})")
  ...
  async def on_command_error(
    ...
    if isinstance(error, (NotOwner, CommandOnCooldown)):
    ...
    elif isinstance(error, (MissingRequiredArgument, MissingRequiredAttachment, BadLiteralArgument)):
      return await ctx.send_help(ctx.command)
    
    elif isinstance(error, CheckFailure):
      if isinstance(error, MissingPermissions):
        return await ctx.wanr(f"You're missing `{' '.join(p for p in error.missing_permissions)}` permission")
      ...
      if check:
        ...
        msg.content = msg.content.replace(
          ctx.invoked_with, check['alias'], 1
        )
  ...
  async def on_message_edit(self: "Luma", before: discord.Message, after: discord.Message):
  ...
  async def disabled_command(self, ctx: Context):
    ...
    if check := await self.db.fetchrow(
      "SELECT * FROM disablecmd WHERE guild_id = $1 AND command = $2", ctx.guild.id, ctx.command.qualified_name
    ):
...
async def getprefix(bot: Luma, message: discord.Message):
  prefix: str = await bot.db.fetchval(
