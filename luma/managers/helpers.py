...
class Context(commands.Context):
  ...
  async def has_reskin(
    ...
    check = await self.bot.db.fetchrow(
      "SELECT * FROM reskin WHERE user_id = $1",
      self.author.id
    )
    if not check:
      return None
  ...
  async def send(self: "Context", *args, **kwargs) -> discord.Message:
    ...
    if patch := kwargs.pop("patch", None):
      ...
      if not reskin:
        ...
        try:
          webhook = next(
            (w for w in await self.channel.webhooks() if w.user.id == self.bot.user.id),
          )
        except StopIteration:
          webhook = await self.channel.create_webhook(name=f"{self.bot.user.name} - reskin")
  ...
  async def error(self: "Context", message: str) -> discord.Message:
    embed = discord.Embed(
      color=0xFF6464,
      description=f":x: {self.author.mention}: {message}"
    )
    return await self.reply(embed=embed)
  ...
  async def get_attachment(self: "Context"):
    ...
    mes = [
      m async for m
      in self.channel.history(limit=10)
      if m.attachments
    ]
    if len(mes) > 0:
  ...
  async def paginator(self: "Context", embeds: List[Union[discord.Embed, str]]) -> discord.Message:
...
class LumaHelp(commands.HelpCommand):
  ...
  async def send_group_help(self: "LumaHelp", group: commands.Group): 
    ...
    for command in group.commands:
      i += 1
      aliases = list(map(lambda a: a.alias, await self.context.bot.db.fetch(
  ...
  async def send_bot_help(self, _):
    ...
    for cog in cogs:
      embeds.append(
        ...
        .set_footer(
          text=f"{len(tuple(cog.walk_commands()))} commands"
        )
      )
