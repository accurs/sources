...
class Notifications(commands.Cog):
    ...
    if channel.type.name == "text":
      for i in ['welcome', 'leave']:
        await self.bot.db.execute(
          f"DELETE FROM {i} WHERE channel_id = $1",
          channel.id
        )
  ...
  async def starboard_disable(self: "Notifications", ctx: Context):
    ...
    await self.bot.db.execute(
      "DELETE FROM starboard_message WHERE guild_id = $1",
      ctx.guild.id
    )
  ...
  @commands.has_guild_permissions(manage_guild=True)
  async def starboard_channel(self: "Notifications", ctx: Context, *, channel: discord.TextChannel):
    """
  ...
  async def starboard_settings(self: "Notifications", ctx: Context):
    ...
    .add_field(
      name="Channel",
      value=ctx.guild.get_channel(result['channel_id']).mention if ctx.guild.get_channel(result['channel_id']) else "N/A"
    )\
    .add_field(
      name="Emoji", 
      value=result['emoji']
  ...
  @commands.has_guild_permissions(manage_guild=True)
  async def welcome_add(self: "Notifications", ctx: Context, channel: discord.TextChannel, *, message: str):
    """
    ...
    await ctx.confirm(f"Added {channel.mention} to welcome with message:\n`{message}`")

  @welcome.command(name="remove")
  @commands.has_guild_permissions(manage_guild=True)
  async def welcome_remove(self: "Notifications", ctx: Context, *, channel: discord.TextChannel):
    """
  ...
  @commands.has_guild_permissions(manage_guild=True)
  async def welcome_test(self: "Notifications", ctx: Context, *,channel: discord.TextChannel):
    """
  ...
  @commands.has_guild_permissions(manage_guild=True)
  async def leave_add(self: "Notifications", ctx: Context, channel: discord.TextChannel, *, message: str):
    """
    Add a leave channel
    """
    await self.bot.db.execute(
    ...
    await ctx.confirm(f"Added leave channel {channel.mention} message:\n`{message}`")

  @leave.command(name="remove")
  @commands.has_guild_permissions(manage_guild=True)
  async def leave_remove(self: "Notifications", ctx: Context, *, channel: discord.TextChannel):
    """
    Remove a leave channel
    """
    if not await self.bot.db.fetchrow(
      "SELECT * FROM leave WHERE guild_id = $1",
      ctx.guild.id
    ):
  ...
  @commands.has_guild_permissions(manage_guild=True)
  async def leave_test(self: "Notifications", ctx: Context, *, channel: discord.TextChannel):
    """
