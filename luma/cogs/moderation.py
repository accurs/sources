...
from managers.validators import ValidTime, ValidNickname, NoStaff, RoleConvert
...
class Moderation(commands.Cog):
  ...
  async def slowmode(
    ...
    *,
    ...
    await ctx.confirm(f"Set slowmode to `{format_timespan(time)}` seconds for {channel.mention}")
  ...
  async def mute(
    ...
    *,
    ...
    await member.timeout(discord.utils.utcnow() + datetime.timedelta(seconds=time), reason=reason + f" muted by {ctx.author}")
    await ctx.confirm(f"Stopped {member.mention}'s yapping for `{format_timespan(time)}`")
  ...
  async def purge_invites(
    ...
    async with self.locks[ctx.channel.id]:
      await ctx.channel.purge(
        ...
        check=lambda m: re.search(r"(https?://)?(www.|canary.|ptb.)?(discord.gg|discordapp.com/invite|discord.com/invite)/?[a-zA-Z0-9]+/?", m.content)
      )
  ...
  async def nuke(
    ...
    async with self.locks[ctx.channel.id]:
      ...
      await ctx.confirm_view(
        f"Are u sure about nukeing this channel?",
        yes_callback,
        no_callback
      )
  ...
  async def thread_lock(
    ...
    *,
    ...
    await channel.edit(
      locked=True,
      reason=f"Thread locked by {ctx.author}"
    )
  ...
  async def thread_unlock(
    ...
    *,
    ...
    await channel.edit(
      locked=False,
      reason=f"Thread unlocked by {ctx.author}"
    )
    await ctx.message.add_reaction("✅")
  ...
  async def thread_delete(
    ...
    *,
    ...
    await channel.delete(
      reason=f"Thread deleted by {ctx.author}"
    )
    await ctx.message.add_reaction("✅")
  ...
  async def thread_name(
    ...
    *,
    ...
    await channel.edit(
      name=name,
      reason=f"Thread renamed by locked by {ctx.author}"
    )
    await ctx.message.add_reaction("✅")
  ...
  async def thread_archive(
    ...
    *,
    ...
    await channel.edit(
      archived=True,
      reason=f"Thread archived by {ctx.author}"
    )
  ...
  async def thread_slowmode(
    ...
    *,
    ...
    await channel.edit(
      slowmode_delay=time,
      reason=f"Thread slowmode set by {ctx.author}"
    )
    await ctx.message.add_reaction("✅")
