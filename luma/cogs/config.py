...
from managers.validators import (
  ValidPermission,
  RoleConvert,
  ValidCommand,
  ValidAlias
...
class Config(commands.Cog):
  ...
  async def fakeperms_add(
    ...
    *,
    ...
    await self.bot.db.execute(
      ...
      permissions = $3 
      """,
      ctx.guild.id, role.id, permissions
    )

    await ctx.confirm(f"Added `{permission}` to the {role.mention}'s fake permissions")
  ...
  async def alias_list(
    ...
    await ctx.paginate(
      [f"**{result['alias']}** is an alias for **{result['command']}**" for result in results],
  ...
  async def poj_add(
    ...
    *,
    ...
    await self.bot.db.execute(
      "INSERT INTO pingonjoin VALUES ($1,$2)",
      ctx.guild.id,
      channel.id
    )
  ...
  async def poj_list(
    ...
    """
    results = await self.bot.db.fetch(
      "SELECT * FROM pingonjoin WHERE guild_id = $1",
      ctx.guild.id
    )
    if not results:
  ...
  @autoresponder.command(
    name="add",
    usage="example ;ar add Luma, hi im luma"
  )
  @commands.has_guild_permissions(manage_guild=True)
  async def ar_add(
    self: "Config",
    ...
    *,
    ...
    await self.bot.db.execute(
      ...
      response = $3 
      """,
      ctx.guild.id, trigger.lower(), responsee
    )
    await ctx.confirm(f"Added trigger `{trigger.lower()}` and response `{responsee}`")
  ...
  async def disablecommand(
    ...
    *,
    ...
    """
    if await self.bot.db.fetchrow(
      "SELECT * FROM disablecmd WHERE guild_id = $1",
      ctx.guild.id
    ):
      return await ctx.error("This command is already disabled")
    
    await self.bot.db.execute(
      "INSERT INTO disablecmd VALUES ($1,$2)",
      ctx.guild.id,
      command
    )
