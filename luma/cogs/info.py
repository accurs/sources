...
class Info(commands.Cog):
  ...
  async def invite(self: "Info", ctx: Context):
    ...
    """
    await ctx.reply(discord.utils.oauth_url(self.bot.user.id, permissions=discord.Permissions(permissions=8)))
  ...
  async def vote(
    ...
    """
    await ctx.reply(f"[Vote](<https://top.gg/bot/1263203971846242317>) for **{self.bot.user.name}**")
