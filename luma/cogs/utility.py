...
class Utility(commands.Cog):
  ...
  async def avatar(self: "Utility", ctx: Context, *, member: discord.User = commands.Author):
    """
  ...
  async def muted(self: "Utility", ctx: Context):
    ...
    await ctx.paginate(
      [f"{member} - <t:{member.timed_out_until.timestamp()}:R>" for member in muted],
  ...
  @app_commands.allowed_installs(guilds=True, users=True)
  @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
  async def userinfo(self: "Utility", ctx: Context, member: Union[discord.Member, discord.User] = commands.Author) -> discord.Message:
    """
    ...
    .set_footer(
      text=f"mutuals: {len(member.mutual_guilds)}"
    )
    if isinstance(member, discord.Member):
  ...
  async def shazam(self: "Utility", ctx: Context, *, attachment: discord.Attachment):
    ...
    try:
      ...
      return await ctx.reply(f"<:shazam:1133831097445269504> {ctx.author.mention}: Found [**{name}**]({link})")
    except:
  ...
  @app_commands.allowed_installs(guilds=True, users=True)
  @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
  async def dominant(self: "Utility", ctx: Context, *, attachment: discord.Attachment):
    """
  ...
  async def snipe(self: "Utility", ctx: Context, *, index = 1):
    ...
    elif re.search(r'(https?://)?(www.|canary.|ptb.)?(discord.(gg|io|me|li)|discordapp.com/invite|discord.com/invite)/?[a-zA-Z0-9]+/?', sniped.content):
    ...
    .set_author(
      ...
      icon_url=sniped.author.display_avatar.url
    )\
    .set_footer(
      text=f"{index}/{len(snipes)}"
    )
    await ctx.reply(embed=embed)
  ...
  async def instagram(
    ...
    x = await self.bot.session.get(
      "https://api.fulcrum.lol/instagram",
