...
class Developer(commands.Cog):
  ...
  async def eval(
    ...
    *,
    ...
    return await ctx.invoke(
      self.bot.get_command("jishaku py"),
      argument=argument
    )
  ...
  async def guilds(
    ...
    await ctx.paginate(
      [f"{g.name} - {g.member_count:,}" for g in servers],
      title="Luma servers"
    )
  ...
  async def edit_avatar(
    ...
    if image:
      ...
      if re.search(r"(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])", image):
  ...
  async def donor_remove(
    ...
    *,
    ...
    await self.bot.db.execute(
      "DELETE FROM donor WHERE user_id = $1",
      member.id
    )
