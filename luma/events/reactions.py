      ...
      if res := await self.bot.db.fetchrow(
        ...
        if channel := self.bot.get_channel(res['channel_id']):
          if res['emoji'] == str(payload.emoji):
            ...
            return await m.edit(content=f"**#{reaction.count}** {payload.emoji}")

  @commands.Cog.listener("on_raw_reaction_add")
  async def on_starboard(
    self: "Reactions",
    payload: discord.RawReactionActionEvent
    ):
      if result := await self.bot.db.fetchrow("SELECT * FROM starboard WHERE guild_id = $1", payload.guild_id):
        if channel := self.bot.get_channel(result.channel_id):
          if emoji := result.emoji:
            if count := result.count:
              if str(payload.emoji) == emoji:
                if not await self.bot.db.fetchrow("SELECT * FROM starboard_message WHERE panel_message_id = $1", payload.message_id):
                  async with self.locks[f"{payload.emoji}-{payload.message_id}"]:
                    ...
                    if message.content != '' or len(message.attachments) > 0:
                      ...
                      if reaction.count >= count:
                        author = self.bot.get_guild(payload.guild_id).get_member(payload.message_author_id)
                        ...
                        if len(message.attachments) > 0:
                          ...
                          if attachment.filename.endswith(tuple(["png", "gif", "jpeg", "jpg"])):
                            embed.set_image(url=attachment.url)
                          elif attachment.filename.endswith(tuple(['mp4', 'mov'])):
                            file = discord.File(
                              await self.bot.bytes(attachment.url),
